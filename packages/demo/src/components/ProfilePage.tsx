...
export function ProfilePage(): JSX.Element {
  ...
  const { policy } = useConsentFlow("sample-policy-1" /* TODO: Get from API */);

  const formData: ConsentFlowFormData = location.state?.formData;

  const getConsentScopes = (type: 'allowed' | 'revoked' /* TODO: Reuse ConsentStatus */, grantedScopes: string[]): ProfileData['consents'] => {
    const common = {
      policy: {
        id: policy?.id ?? 'sample-policy-1',
    ...
    if (type === 'revoked') {
      result = {
        ...
        scopes: (policy?.scopes ?? [])
          .filter(scope => !grantedScopes?.includes(scope.key))
          .map(scope => ({
            id: scope.key,
  ...
  const profileData: ProfileData = !formData ? sampleProfileData : {
    id: 'user123', // TODO: Get from API
    name: formData?.name,
    email: 'email@example.com',
    role: {
      id: formData?.roleId,
      label: formData?.isProxy ? 'Parent / Guardian / Representative' : 'Myself' // TODO: Get label from API
    },
    managedSubjects: formData?.managedSubjects.map((subject) => {
      let ageGroupLabel = subject.ageRangeId === 'under13' ? 'Child' : subject.ageRangeId === '13-17' ? 'Teen' : 'Adult'; // TODO: Get label from API

      if (subject.age === undefined) ageGroupLabel = 'N/A';
      else if (subject.age < 1) ageGroupLabel = `${ageGroupLabel} (less than 1 year old)`;
      else if (subject.age === 1) ageGroupLabel = `${ageGroupLabel} (1 year old)`;
  ...
  const handleProfileUpdate = (profileId: string, updates: Partial<ProfileData>): void => {
